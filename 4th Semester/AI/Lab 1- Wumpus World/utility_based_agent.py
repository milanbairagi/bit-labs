...
class UtilityBasedAgent:
    ...
    def action(self, percept, env):
        ...
        # Explore: prioritize adjacent safe unvisited cells
        x, y = env.agent_pos
        for dx, dy in [(0,1), (1,0), (0,-1), (-1,0)]:
            adj = (x+dx, y+dy)
        ...
        # No adjacent unvisited safe cells, use pathfinding to reach any safe unvisited cell
        ...
        # Default: turn right if stuck (shouldn't happen with good exploration)
        return "TURN_RIGHT"
    
    def find_path_through_visited(self, start, goal):
        """BFS to find path from start to goal through visited cells.
        The goal can be unvisited, but the path goes through visited cells."""
        if start == goal:
            return [start]
        ...
        while queue:
            ...
            for dx, dy in [(0,1), (1,0), (0,-1), (-1,0)]:
                ...
                # Continue through visited cells
                if next_pos in self.visited and next_pos not in seen:
                    seen.add(next_pos)
                    queue.append((next_pos, path + [next_pos]))
        return []